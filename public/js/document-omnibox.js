(function () {
  ...
  function createDocumentOmnibox(config) {
    ...
    const presetName =
      typeof config.preset === 'string' ? config.preset : 'default';
    const preset =
      DOCUMENT_OMNIBOX_PRESETS[presetName] || DOCUMENT_OMNIBOX_PRESETS.default;
    ...
    const settings = {
      ...
      formatDate:
        typeof config.formatDate === 'function'
          ? config.formatDate
          : defaultFormatDate,
      selectedStatusFormatter:
        typeof config.selectedStatusFormatter === 'function'
          ? config.selectedStatusFormatter
          : preset.selectedStatusFormatter,
      availableStatusFormatter:
        typeof config.availableStatusFormatter === 'function'
      ...
      initialStatusMessage:
        config.initialStatusMessage || preset.initialStatusMessage,
      ...
      filterResults:
        typeof config.filterResults === 'function'
          ? config.filterResults
          : null,
    ...
    function updateActiveResultHighlight() {
      const items = resultsElement.querySelectorAll(
        `.${settings.resultItemClass}`
      );
    ...
    function removeResultRow(doc) {
      ...
      // The remaining rows shifted up, so re-apply the highlight.
    ...
    function renderResults(documents) {
      ...
      documents.forEach((doc, index) => {
        ...
        metaRow.appendChild(
          createMetaPill(
            (doc && doc.correspondent) || 'No correspondent',
            'correspondent'
          )
        );
        metaRow.appendChild(
          createMetaPill(settings.formatDate(doc && doc.created), 'date')
        );
        metaRow.appendChild(
          createMetaPill(`ID ${(doc && doc.id) || '-'}`, 'id')
        );
    ...
    async function load(searchTerm, options) {
      ...
      try {
        const response = await fetch(
          `${settings.fetchUrl}?${params.toString()}`,
          {
            method: 'GET',
        ...
        let documents = Array.isArray(
          payload && payload.data && payload.data.documents
        )
        ...
        if (documents.length === 0) {
          ...
          const availableStatus = settings.availableStatusFormatter(
            documents.length
          );
